...
void DataShareMgr::InsertSalt(int32 socket, std::string salt) {
	//  base_logic::WLockGd lk(lock_);
  salt_map_[socket] = salt;
}

std::string DataShareMgr::GetSalt(int32 socket) {
	//  base_logic::WLockGd lk(lock_);
  SaltMap::iterator it = salt_map_.find(socket);
  if (it != salt_map_.end()) {
    return it->second;
  }
  return "";
...
void DataShareMgr::EraseSalt(int32 socket) {
	//base_logic::WLockGd lk(lock_);
  SaltMap::iterator it = salt_map_.find(socket);
  if (it != salt_map_.end())
    salt_map_.erase(it);
